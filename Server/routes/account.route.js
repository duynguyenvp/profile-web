...
router.get(
  ...
  (req, res) => {
    request.post(
      { url: `${System.API}/User/SiginOAuth`, formData: req.user },
...
router.get("/login:returnUrl?", checkAuth, (req, res) => {
  ...
  const resources = getResources("login");
  res.send(
    templateLogin({
      title: `Đăng nhập`,
      styles: [...css].join(""),
...
router.get("/register:returnUrl?", checkAuth, (req, res) => {
  ...
  const resources = getResources("register");
  res.send(
    templateRegister({
      title: `Register`,
      context: JSON.stringify({ message: message }),
...
router.post("/signup", (req, res) => {
  ...
  request.post(
    { url: `${System.API}/User/Sigup`, formData: formData },
    function optionalCallback(err, httpResponse, body) {
      if (err) {
        res.send(err);
        return console.error("login failed:", err);
...
router.post("/signin", (req, res) => {
  ...
  request.post(
    { url: `${System.API}/User/Sigin`, formData: formData },
    function optionalCallback(err, httpResponse, body) {
      if (err) {
        res.send(err);
...
const Login = (url, data) => {
  return new Promise((resolve, reject) => {
    request.post(
      { url: `${System.API}/${url}`, formData: data },
      function optionalCallback(err, httpResponse, body) {
        if (err) {
          reject(err);
          return;
