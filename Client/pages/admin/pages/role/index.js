...
import { Row, Col, Table, Input, Button, notification, Modal, Popover } from 'antd'
const { Column } = Table
const { confirm } = Modal;
...
const getPopoverContent = services => {
    let result = (<div>
        {
            services && services.map((service, index) => <p key={index}>{service.serviceName}</p>)
        }
    </div>)
    return result
}
        ...
        <Fragment>
            ...
            <Row style={{ marginTop: 30 }}>
                ...
                >
                    ...
                    <Column title="Mô tả" dataIndex="roleDescription" key="roleDescription" />
                    <Column title="Cấp bậc" dataIndex="roleLevel" key="roleLevel" />
                    <Column title="Dịch vụ đã gán"
                        dataIndex="services"
                        key="services"
                        render={(text, record) => (
                            <Popover content={popovercontent} title="Danh sách dịch vụ" trigger="click">
                                <Button onClick={e => {
                                    e.preventDefault();
                                    e.stopPropagation();
                                    setPopovercontent(getPopoverContent(record.services))
                                }}>{record.services.length} Dịch vụ</Button>
                            </Popover>
                        )} />
                    <Column
                        title=""
