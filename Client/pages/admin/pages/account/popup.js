...
const AccountForm = ({ onClose, account, dataRoles }) => {
    const onFinish = values => {
        ...
        getApiInstance().postWithFormAuth({
            ...
            if (res.successful) {
                ...
                addOrUpdateAccount({ ...account, role: role.roleName, roleId: role.id })
                onClose()
            } else {
    ...
    const { role, roleId, username, email } = account || {}
    const options = dataRoles.map((_role, index) => {
        return <Option key={index} value={_role.id}>{_role.roleName}</Option>
    }) || [];
    return (
...
const AccountPopup = ({ visible, onClose, account, dataRoles }) => {
    return (
        <Drawer
