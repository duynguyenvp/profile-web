...
const ChangePassword = ({ visible, onClose, sigout }) => {
    ...
    const handleOk = e => {
        if (validate()) {
            ...
            if (!auth) {
                openNotificationWithIcon('error', 'Đã xảy ra lỗi, vui lòng đăng nhập lại!')
                return
            }
            getApiInstance().postWithFormAuth({
                ...
                if (successful) {
                    const descriptioin = <p>{result || 'Đổi mật khẩu thành công!'} <br />Vui lòng đăng nhập lại sau 3s.</p>
