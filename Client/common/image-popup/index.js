...
const ImageItem = ({ src, callback, remove, isSelected }) => {
    let avatarStyle = {
        backgroundImage: `url("${src || require('../../common-resources/images/imageDefault.jpg')}")`
    }
    ...
    return (
        <div className={`image-item ${isSelected ? 'selected' : ''}`} style={avatarStyle} onClick={callback}>
            <div className="preview-image" onClick={handlePreview}>
                <i className="material-icons">zoom_in</i>
            </div>
            <div className="icon-remove" onClick={handleRemove}></div>
        </div>
    )
}
...
const ImagePopup = props => {
    ...
    const handleUploadFromUrl = () => {
        ...
        testImage(url).then(res => {
            if (res === 'success') {
                ...
                addAlert({ type: 'error', message: 'Không thể tải ảnh. Vui lòng kiểm tra lại dường dẫn.' })
                setUrl('')
            }
        }).catch(err => {
            addAlert({ type: 'error', message: 'Không thể tải ảnh. Vui lòng kiểm tra lại dường dẫn.' })
    ...
    let avatarStyle = {
        backgroundImage: `url("${selected || require('../../common-resources/images/imageDefault.jpg')}")`
    }
    return (
                ...
                ({ onClose }) =>
                    <div className="image-popup-wrapper">
                        <div className="image-popup-body">
                            <div className="image-upload-area">
                                ...
                                <label htmlFor="input-image" className="input-button">
                                    <i className="material-icons">cloud_upload</i> Upload ảnh mới
                                </label>
                                <input type="file"
                                    id="input-image"
                                ...
                                <div className="form-filter">
                                    ...
                                    <div className="filter-item controls">
                                        <button className="btnPrimary margin-16px" onClick={() => { handleUploadFromUrl() }}>Upload</button>
                                        <button className="btnDanger" onClick={() => { setUrl('') }}>Xóa</button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="filter-item flex-end">
                            {
                                selected && <button className="btnPrimary margin-16px" onClick={
                                    () => {
                                        callback(selected);
                                        onClose()
                                    }
